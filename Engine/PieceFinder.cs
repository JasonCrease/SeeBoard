...
namespace Engine
{
    public class PieceFinder
    {
        ...
        private void FindPiece(int x, int y)
        {
            ...
            srcs[0] = new PointF(quad.p[0].X - topSkew, quad.p[0].Y);
            srcs[1] = new PointF(quad.p[1].X - topSkew, quad.p[1].Y);
            srcs[2] = quad.p[2];
            srcs[3] = quad.p[3];
            ...
            dsts[2] = new PointF(64, 64);
            dsts[3] = new PointF(0, 64);

            HomographyMatrix warpMat = CameraCalibration.GetPerspectiveTransform(srcs, dsts);
            m_PieceImages[x,y] = m_Image.WarpPerspective(warpMat, 64, 64,
